...
(defconst pragmatapro-lig-alist
  '(("\\(\\[ERROR\\]\\)" #Xe380)
    ...
    ("\\(\\]\\]\\)" #Xea81)
    ("\\(\">\\)" #Xea90)))
(defconst pragmatapro-font-lock-keywords-alist
  (mapcar (lambda (regex-char-pair)
	    `(,(car regex-char-pair)
	      (0 (prog1 ()
             (compose-region
              (match-beginning 1) (match-end 1)
              ,(concat "	" (list (decode-char 'ucs
                                               (cadr regex-char-pair)))))))))
          (sort pragmatapro-lig-alist
                (lambda (x y) (< (length (car x)) (length (car y)))))))
(defun add-pragmatapro-symbol-keywords ()
  (font-lock-add-keywords nil pragmatapro-font-lock-keywords-alist))
